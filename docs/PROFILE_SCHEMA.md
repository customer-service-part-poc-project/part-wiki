...
`signals.badge` 는 `extract_signals.py` 가 넣는 원값이다 — `{marker, ratio, count, kind}`.
개인 페이지는 `fun.speech_badge` 의 이름표를, **목록 카드는 이 원값을 `단정 3.5배` 처럼 짧게** 쓴다.
긴 이름표를 목록에 넣으면 별명과 겹쳐 보인다.
