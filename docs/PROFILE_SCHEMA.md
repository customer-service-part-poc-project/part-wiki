...
| `blood_type` | `없음(무작위)` | **데이터 신호가 0이라는 걸 밝히고 억지 논리를 한 줄 붙이는 게 규칙** |
| `how_to_talk` | (강도 없음) | 관측된 응답 패턴·활동 시간대·말투 |

> 혈액형을 "없음(무작위)" 이라고 대놓고 쓰는 이유: 확신에 찬 오답이 회람되는 것보다 낫고,
> 근거가 전혀 없다고 밝히는 쪽이 실제로 더 웃긴다. (`../members/docs/PRIVACY.md`)
...
4. **`fun` 의 mbti·age_band·blood_type 에 `strength` 필드 존재**
